...
app.controller('PrintController', ['$scope', '$window', '$compile', '$http', '$filter', '$q', function ($scope, $window, $compile, $http, $filter, $q) {
        ...
        $scope.calculateListDays = calculateListDays;
        ...
        var a = $http.get('data-ptr/days.json').
                success(function (data) {
                    ...
                    iEl.empty();
                    var sa = $compile("<table class='table table-striped-special' ui-jq='dataTable' ui-options='dataTableOpt'><thead><tr><th style=\"display:none\"></th><th>No.</th><th>Fecha<span class=\"text-danger\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Usar este formato de fecha para ordenar correctamente\">*</span></th><th>Fecha</th><th>D&iacute;a</th><th>P&aacute;ginas<span class=\"text-danger\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Para el caso de las impresoras que imprimen Doble Cara\">*</span></th><th>Hojas</th><th>PrintJobs</th></tr></thead><tbody><tr ng-click=\"calculateListDays(day)\" ng-class=\"{'row-selected': day.checked}\" data-ng-repeat=\"day in daysOfWork| orderBy: '-date'\"><td style=\"display:none\"><input type=\"checkbox\" ng-model=\"day.checked\" ng-checked=\"day.checked\"></td><td>{{$index + 1}}</td><td>{{day.date}}</td><td><a style=\"cursor: pointer\" ng-click=\"selectDay(day.date)\" title='Click para ver detalles'>{{day.date| date: 'MMMM d, y'}}  <span ng-if=\"day.date == todayK\" class='label label-primary'>Hoy</span></a></td><td>{{day.day}}</td><td>{{day.total}}</td><td>{{day.totalHojas}}</td><td>{{day.printjob}}</td></tr></tbody></table>")($scope);
                    iEl.append(sa);
...
//            var nameFile = type === 'month' ? 'sheet-count-' + yearAndMonth + '.txt' : 'cont-all-time-printcontrol.txt'
        ...
        function selectDay(date) {
            ...
            var a = $http.get('data-ptr/days.json').
                    success(function (data) {
                        $scope.daysOfWork = data;
                        $scope.daysOfWork.forEach(element => {
            ...
            if (pos !== -1) {
                ...
                promise1.then(function (result) {
                    ...
                    $scope.logsdaySelected.forEach(element => {
                        element.checked = false;
                    });
                    ...
                    var iEl = angular.element(document.querySelector('#tabletoremove'));
                    iEl.empty();
                    var sa = $compile("<table class='table table-striped-special' ui-jq='dataTable' ui-options='dataTableOptDay'><thead><tr><th style=\"display:none\"></th><th>No.</th><th>Hora</th><th>Usuario</th><th>P&aacute;ginas</th><th>Copias</th><th>Total P&aacute;g</th><th>Total Hojas</th><th>Impresora</th><th>Documento</th><th>PC</th><th>Formato</th><th>Color</th><th>Doble Cara</th></tr></thead><tbody><tr ng-click=\"calculate(day)\" ng-class=\"{'row-selected': day.checked}\" data-ng-repeat=\"day in logsdaySelected | orderBy: '-time'\">\n\
...
<td>{{day.duplex===\"NOT DUPLEX\" ? \"No\" : \"S&iacute;\"}}</td>\n\
        ...
        function calculate(day){
            $scope.totalPagesReport = 0;
            $scope.totalSheetsReport = 0;
            console.info("day specific");
            console.info($scope.listRecordToCalculate);
            console.info("=============");
        ...
        function calculateListDays(day){
            $scope.totalPagesReport = 0;
            $scope.totalSheetsReport = 0;
            console.info("list days");
            console.info($scope.listRecordToCalculate);
            console.info("=============");
            if(day.checked){
                $scope.listRecordToCalculate.splice($scope.listRecordToCalculate.indexOf(day),1);
            ...
            for (var i = 0; i < $scope.listRecordToCalculate.length; i++) {         
                var totalPages = $scope.listRecordToCalculate[i].total;
                $scope.listRecordToCalculate[i].totalPages = totalPages;
                $scope.totalPagesReport = $scope.totalPagesReport + totalPages;

                var sheets = $scope.listRecordToCalculate[i].totalHojas;
                $scope.listRecordToCalculate[i].sheets = sheets;
                $scope.totalSheetsReport = $scope.totalSheetsReport + sheets;
            }
            console.log($scope.listRecordToCalculate);
